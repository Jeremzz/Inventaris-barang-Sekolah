    ...
    <title>Dana BOS - SMKN 2 Kendari</title>
    ...
    <div class="flex flex-1 relative">
        ...
        <nav class="p-4 space-y-2">
            ...
            <a href="/dana-bos" class="block py-2 px-4 rounded bg-indigo-100 text-indigo-700 font-semibold">Dana BOS</a>
        ...
        <header class="bg-indigo-500 h-20 flex items-center justify-between px-6 text-white shadow z-10">
            ...
            <span class="text-xl font-bold">Dana BOS</span>
            <span class="text-sm">Halo, Administrator</span>
        </header>

        <main class="p-6">
            <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800 tracking-wide">Manajemen Dana BOS</h2>
            <button class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded text-sm shadow font-medium"
                onclick="openModal('modalTambahDana')">+ Tambah Data</button>
            </div>
            <!-- Tombol Cari Surat -->
        ...
        <button onclick="openModal('modalCariTanggal')" type="button"
            ...
            <table class="min-w-full table-auto">
                ...
                @forelse ($danaBos as $index => $bos)
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-center">{{ $index + 1 }}</td>
                    <td class="px-4 py-3 text-center">{{ \Carbon\Carbon::parse($bos->tanggal_masuk)->format('d-m-Y') }}</td>
                    <td class="px-4 py-3 text-center">{{ $bos->sumber }}</td>
                    <td class="px-4 py-3 text-center">
                    @if ($bos->foto && file_exists(public_path('storage/dana_bos_foto/' . $bos->foto)))

                        <img src="{{ asset('storage/dana_bos_foto/' . $bos->foto) }}"
                        class="w-20 h-20 object-cover rounded mx-auto cursor-pointer border" onclick="showFullImage('{{ asset('storage/dana_bos_foto/' . $bos->foto) }}')">
                    @else
                    ...
                    <div class="flex justify-center gap-3">
                        <!-- Tombol Edit -->
                        <button onclick='openEditModal(@json($bos))' class="text-yellow-500 hover:text-yellow-600" title="Edit">
                        ...
                        <!-- Tombol Hapus -->
                        <form action="{{ route('dana-bos.destroy', $bos->id) }}" method="POST"
                ...
                @empty
                <tr>
                    <td colspan="5" class="text-center py-4 text-gray-500">Belum ada data surat dana BOS.</td>
                </tr>
                @endforelse
                </tbody>
            </table>
    ...
    <!-- Modal Tambah Dana BOS -->
    <div id="modalTambahDana"
        ...
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-bold text-gray-800">Tambah Dana BOS</h3>
            <button onclick="closeModal('modalTambahDana')" class="text-gray-500 hover:text-gray-700 text-xl">&times;</button>
        </div>
        <form action="{{ route('dana-bos.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4">
            <div>
            ...
            <div>
                ...
                <input type="text" name="sumber" placeholder="Contoh: Kemendikbud" required
                class="w-full border border-gray-300 rounded-md px-3 py-2">
            </div>
            <div>
            ...
            <button type="button" onclick="closeModal('modalTambahDana')"
    ...
    <!-- Modal Edit Dana BOS -->
    <div id="editDanaModal" class="fixed inset-0 bg-black bg-opacity-40 hidden z-50 items-center justify-center px-4 md:px-0">
    <div class="bg-white w-full max-w-xl rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-4">
        <h3 class="text-xl font-bold text-gray-800">Edit Dana BOS</h3>
        <button onclick="closeModal('editDanaModal')" class="text-gray-500 hover:text-gray-700 text-xl">&times;</button>
        </div>
        <form id="editDanaForm" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            <div>
            ...
            <div>
            <label class="block text-sm font-medium text-gray-700">Sumber</label>
            <input type="text" name="sumber" id="edit_sumber"
                class="w-full border border-gray-300 rounded-md px-3 py-2" required>
            </div>
            <div>
        ...
        <div class="mt-6 flex justify-end">
            <button type="button" onclick="closeModal('editDanaModal')"
            class="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded text-gray-700 mr-2">Batal</button>
            <button type="submit"
    ...
    <!-- Modal Cari Tanggal -->
    <div id="modalCariTanggal" class="fixed inset-0 bg-gray-700 bg-opacity-60 hidden z-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-xl">
        <div class="flex justify-between items-center border-b pb-3">
        <h3 class="text-lg font-bold text-gray-800">Cari Surat Berdasarkan Tanggal</h3>
        <button onclick="closeModal('modalCariTanggal')" class="text-gray-500 hover:text-gray-700 text-xl">&times;</button>
        </div>

        <form id="formCariTanggal" class="mt-4">
        <label for="tanggal" class="block text-sm font-medium text-gray-700 mb-1">Pilih Tanggal Masuk</label>
        <input type="date" id="tanggal" name="tanggal"
            class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">

        <div class="flex justify-end mt-4">
            <button type="button" onclick="closeModal('modalCariTanggal')"
            class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded mr-2">Tutup</button>
            <button type="submit"
            class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded">Cari</button>
        </div>
        </form>

        <!-- Hasil pencarian -->
        <div id="hasilCariTanggal" class="mt-6 hidden">
        <h4 class="text-md font-semibold mb-2">Hasil:</h4>
        <table class="w-full text-sm border">
            ...
            <tr>
                ...
                <th class="p-2 border">Foto Surat</th>
            </tr>
            </thead>
            <tbody id="hasilTanggalBody" class="text-gray-700">
            </tbody>
        </table>
        </div>
    </div>
    ...
    <script>
        ...
        function openEditModal(bos) {
        document.getElementById('edit_tanggal_masuk').value = bos.tanggal_masuk;
        document.getElementById('edit_sumber').value = bos.sumber;

        const form = document.getElementById('editDanaForm');
        form.action = `/dana-bos/${bos.id}`; // pastikan route ini sesuai dengan route update()

        openModal('editDanaModal');
        }
        ...
        fetch(`/dana-bos/cari?tanggal=${encodeURIComponent(tanggal)}`)
                        ...
                        tbody.innerHTML += `
                            <tr>
                                ...
                                <td class="p-2 border text-center">
                                    ${item.foto
                                        ? `<img src="/storage/dana_bos_foto/${item.foto}" alt="Surat" class="h-12 inline cursor-pointer" onclick="showFullImage('/storage/dana_bos_foto/${item.foto}')">`
                                        : '<span class="italic text-gray-400">Tidak ada</span>'
                                    }
                                </td>
                            </tr>`;
