...
<head>
  ...
  <title>Halaman DUDI - SMKN 2 Kendari</title>
...
<body class="bg-gray-100 min-h-screen flex flex-col">
  ...
  <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
    <h2 class="text-2xl md:text-2xl font-bold text-gray-800 tracking-wide">Manajemen Kerjasama DUDI</h2>
    <div class="flex flex-col gap-3 w-full md:w-auto">
      <!-- Form Tambah Perusahaan -->
      <form action="{{ route('dudi.store') }}" method="POST" class="flex flex-col sm:flex-row gap-2">
    @csrf
    ...
    <form method="POST" action="{{ route('jurusan.destroy') }}">
        ...
        <table class="min-w-full table-auto">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <input type="checkbox" name="delete_ids[]" value="{{ $dudi->id }}" class="scale-110 checkbox-item">
                </td>
            ...
            @empty
            <tr>
                <td colspan="3" class="text-center text-gray-500 py-4 text-sm">Belum ada perusahaan</td>
            </tr>
            @endforelse
            </tbody>
      </table>
    ...
    <div class="mt-4 text-left">
      <button type="submit"
        onclick="return confirm('Apakah Anda yakin ingin menghapus perusahaan yang dipilih?')"
    ...
    <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
        ...
        <h3 class="text-lg font-semibold text-gray-800">Cari Perusahaan Berdasarkan Nama</h3>
        ...
        <label class="block text-sm font-medium text-gray-700 mb-1">Masukkan Nama Perusahaan</label>
        <input type="text" name="cari" id="cari" placeholder="Contoh: PT Maju Jaya"
        ...
        <thead class="bg-gray-100 text-left">
            ...
            <th class="p-2 border">Nama Perusahaan</th>
            </tr>
        </thead>
...
document.getElementById('formCariBarang').addEventListener('submit', function(e) {
    ...
    fetch(`/dudi/cari/ajax?query=${encodeURIComponent(keyword)}`)
        .then(response => response.json())
        .then(data => {
            tbody.innerHTML = '';

            if (data.length === 0) {
                tbody.innerHTML = `<tr><td colspan="2" class="text-center p-2 text-gray-500">Tidak ditemukan</td></tr>`;
            } else {
                data.forEach((dudi, index) => {
                    tbody.innerHTML += `
                        <tr>
                            <td class="border p-2">${index + 1}</td>
                            <td class="border p-2">
                                <a href="/dudi/${dudi.id}" class="text-indigo-600 hover:underline font-semibold">
                                    ${dudi.nama_perusahaan}
                                </a>
                            </td>
                        </tr>
        ...
        .catch(error => {
            console.error('Gagal mencari perusahaan:', error);
        });
});
