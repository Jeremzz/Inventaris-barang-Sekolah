...
<head>
    ...
    <title>Detail Jurusan {{ $jurusan->nama_jurusan }} - SMKN 2 Kendari</title>
    ...
    <body class="bg-gray-100">
        ...
        <aside id="sidebar" class="bg-white shadow-md w-64 min-h-screen fixed md:relative z-40">
        ...
        <main class="p-6 space-y-6">
            ...
            <h2 class="text-xl font-bold text-gray-800">Daftar Barang Jurusan: {{ $jurusan->nama_jurusan }}</h2>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('jurusan.index') }}"
                class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded shadow-sm text-sm flex items-center gap-2">
                ← Kembali
                </a>
                <button onclick="window.print()"
                class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded shadow-sm text-sm flex items-center gap-2">
                Cetak Laporan
                </button>
            ...
            <table class="min-w-full table-auto">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-center">{{ $barang->kode_seri }}</td>
                    <td class="px-4 py-3 text-center">{{ $barang->tahun_pengadaan }}</td>
                    <td class="px-4 py-3 text-center">{{ $barang->kondisi }}</td>
                    <td class="px-4 py-3 text-center">
                    @if ($barang->foto)
                        <div class="flex justify-center">
                        <img src="{{ asset('storage/barang_fotos/' . $barang->foto) }}"
                            alt="Foto {{ $barang->nama_barang }}"
                            class="w-20 h-20 object-cover rounded cursor-pointer border hover:shadow-lg transition"
                            onclick="openImageModal('{{ asset('storage/barang_fotos/' . $barang->foto) }}')">
                        </div>
                    @else
                    ...
                    <div class="flex justify-center gap-2">
                        <button onclick='openEditModal(@json($barang))' class="text-yellow-500 hover:text-yellow-600">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 512 512"><path d="M471.6 21.7c-21.9-21.9-57.3-21.9-79.2 0L362.3 51.7l97.9 97.9 30.1-30.1c21.9-21.9 21.9-57.3 0-79.2L471.6 21.7zm-299.2 220c-6.1 6.1-10.8 13.6-13.5 21.9l-29.6 88.8c-2.9 8.6-.6 18.1 5.8 24.6s15.9 8.7 24.6 5.8l88.8-29.6c8.2-2.7 15.7-7.4 21.9-13.5L437.7 172.3 339.7 74.3 172.4 241.7z"/></svg>
                                ...
                                <form action="{{ route('barang.destroy', $barang->id) }}"
                                    method="POST" onsubmit="return confirm('Yakin ingin menghapus barang ini?')" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                            <button type="submit" class="text-red-600 hover:text-red-700">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 448 512"><path d="M135.2 17.7L128 32 32 32C14.3 32 0 46.3 0 64S14.3 96 32 96l384 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-96 0-7.2-14.3C307.4 6.8 296.3 0 284.2 0L163.8 0c-12.1 0-23.2 6.8-28.6 17.7zM416 128L32 128 53.2 467c1.6 25.3 22.6 45 47.9 45l245.8 0c25.3 0 46.3-19.7 47.9-45L416 128z"/></svg>
                        </button>
                        </form>
                    </div>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="7" class="text-center py-4 text-gray-500">Belum ada barang untuk jurusan ini.</td>
                </tr>
                @endforelse
                </tbody>
            </table>
            ...
            <img id="modalImage" src="" alt="Perbesar Foto" class="max-h-[90vh] max-w-[90vw] rounded shadow-lg">
                                ...
                                <form action="{{ route('barang.store') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <option value="{{ $j->id }}" {{ $j->id == $jurusan->id ? 'selected' : '' }}>
                                            {{ $j->nama_jurusan }}
                                            </option>
                                            @endforeach
                                            </select>
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="nama_barang" id="nama_barang" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Contoh: Komputer PC">
                                        </div>
                                        <div>
                                            <label for="kode_seri" class="block text-sm font-medium text-gray-700">Kode Seri / Inventaris</label>
                                            <input type="text" name="kode_seri" id="kode_seri" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Contoh: 346543333">
                                        </div>
                                        <div>
                                            <label for="tahun_pengadaan" class="block text-sm font-medium text-gray-700">Tahun Pengadaan</label>
                                            <input type="number" name="tahun_pengadaan" id="tahun_pengadaan" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Contoh: 2024" min="1990" max="{{ date('Y') }}">
                                        </div>
                                        <div>
                                            <label for="kondisi" class="block text-sm font-medium text-gray-700">Kondisi</label>
                                            <select name="kondisi" id="kondisi" required class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                <option>Baik</option>
                                    ...
                                    <div class="mt-6 flex justify-end">
                                        <button type="button" onclick="closeModal('tambahBarangModal')" class="bg-gray-200 hover:bg-gray-300 text-gray-800 py-2 px-4 rounded mr-2">
                                            Batal
                                        </button>
                                ...
                                <form id="editBarangForm" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <input type="text" name="nama_barang" id="edit_nama_barang" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Kode Seri</label>
                                            <input type="text" name="kode_seri" id="edit_kode_seri" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Tahun Pengadaan</label>
                                            <input type="number" name="tahun_pengadaan" id="edit_tahun_pengadaan" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Kondisi</label>
                                            <select name="kondisi" id="edit_kondisi" required class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md">
                                                <option>Baik</option>
                                                <option>Rusak Ringan</option>
                                                <option>Rusak Berat</option>
                                            </select>
                                    ...
                                    <div class="mt-6 flex justify-end">
                                        <button type="button" onclick="closeModal('editBarangModal')" class="bg-gray-200 hover:bg-gray-300 text-gray-800 py-2 px-4 rounded mr-2">
                                            Batal
                                        </button>
                ...
                <script>
                    ...
                    function openEditModal(barang) {
                        ...
                        document.getElementById('edit_kode_seri').value = barang.kode_seri;
                        document.getElementById('edit_tahun_pengadaan').value = barang.tahun_pengadaan;
